    ...
    <v-layout align-center column fill-height mt-3>
      <section v-if="!startScan && !scanned">
        <v-flex xs12>
          ...
          <span class="display-1"
            >This device requires an owner before it can be used</span
          >
          <!-- </v-alert> -->
        </v-flex>
        <v-flex xs12 sm12>
      ...
      <v-dialog v-model="clockOut" max-width="290">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" flat="flat" @click="clockUserOut"
              >Clock out</v-btn
            >
            <v-btn color="green darken-1" flat="flat" @click="goHome"
              >Cancel</v-btn
            >
          </v-card-actions>
        </v-card>
...
export default {
  ...
  mounted() {
  this.$geb.getFilteredBus({id:'system-ready'}).subscribe(data => {
